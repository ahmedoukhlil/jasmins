    ...
    <div class="bloc-patient">
        <table class="bloc-patient-table">
            ...
            <tr>
                ...
                <td class="value" colspan="3">{{ $mode->LibPaie ?? ($operation->TypePAie ?? 'N/A') }}</td>
            </tr>
        </table>
    </div>
        ...
        <tr>
            <td>
                @if($operation->MontantOperation < 0)
                    Remboursement facture N°{{ $facture->Nfacture ?? 'N/A' }}
                @elseif($operation->MontantOperation > $facture->TotFacture)
                    Acompte facture N°{{ $facture->Nfacture ?? 'N/A' }}
                @else
                    Paiement facture N°{{ $facture->Nfacture ?? 'N/A' }}
                @endif
            </td>
            <td>{{ number_format(abs($operation->MontantOperation ?? 0), 2) }} MRU</td>
        </tr>
        </tbody>
    </table>
