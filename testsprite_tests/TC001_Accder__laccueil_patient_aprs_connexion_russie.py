...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to the HTTPS login page at /login (https://localhost/cabinetsavwa/public/login).
        await page.goto("https://localhost/cabinetsavwa/public/login")
        
        # -> Fill the Identifiant field with 'moctar', fill the password field with '1234', then submit the form (click 'Se connecter').
        ...
        frame = context.pages[-1]
        assert await frame.locator("xpath=//*[contains(., 'Accueil patient')]").nth(0).is_visible(), "The Accueil patient page should be displayed after login"
